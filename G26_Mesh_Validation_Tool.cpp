...
#include "MarlinConfig.h"
  ...
  bool unified_bed_leveling::parse_G26_parameters() {
    ...
    if (parser.seen('Q')) {
      if (parser.has_value()) {
        ...
        if (!WITHIN(g26_retraction_multiplier, 0.05, 15.0)) {
          SERIAL_PROTOCOLLNPGM("?Retraction Multiplier not plausible.");
          return UBL_ERR;
        }
      }
      else {
        SERIAL_PROTOCOLLNPGM("?Retraction Multiplier NOT specified.");
        return UBL_ERR;
      }
    }
    ...
    if (parser.seen('S')) {
      ...
      if (!WITHIN(g26_nozzle, 0.1, 1.0)) {
        SERIAL_PROTOCOLLNPGM("?Nozzle size not plausible.");
        return UBL_ERR;
      }
    }
    ...
    if (parser.seen('P')) {
      if (!parser.has_value())
        ...
        if (!WITHIN(g26_prime_length, 0.0, 25.0)) {
          SERIAL_PROTOCOLLNPGM("?Prime length not plausible.");
          return UBL_ERR;
        }
      }
    }
    ...
    if (parser.seen('F')) {
      ...
      if (!WITHIN(g26_filament_diameter, 1.0, 4.0)) {
        SERIAL_PROTOCOLLNPGM("?Filament size not plausible.");
        return UBL_ERR;
      }
    }
    ...
    if (parser.seen('H')) {
      ...
      if (!WITHIN(g26_hotend_temp, 165, 280)) {
        SERIAL_PROTOCOLLNPGM("?Nozzle temperature not plausible.");
        return UBL_ERR;
      }
    }
    ...
    if (!position_is_reachable_xy(g26_x_pos, g26_y_pos)) {
      SERIAL_PROTOCOLLNPGM("?X,Y coordinate out of bounds.");
      return UBL_ERR;
    }
  ...
  bool unified_bed_leveling::turn_on_heaters() {
    millis_t next = millis() + 5000UL;
        ...
        lcd_setstatusPGM(PSTR("G26 Heating Nozz."), 99);
        lcd_quick_feedback();
      #endif
    #endif
  ...
  bool unified_bed_leveling::prime_nozzle() {
    ...
    if (g26_prime_flag == -1) {  // The user wants to control how much filament gets purged
      ...
      #if ENABLED(ULTRA_LCD)
        strcpy_P(lcd_status_message, PSTR("Primed")); // We can't do lcd_setstatusPGM() without having it continue;
                                                            // So...  We cheat to get a message up.
        lcd_setstatusPGM(PSTR("Primed"), 99);
        lcd_quick_feedback();
      #endif
